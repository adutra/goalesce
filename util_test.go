...
func Test_checkTypesMatchWithKind(t *testing.T) {
	tests := []struct {
		name    string
		v1      reflect.Value
		v2      reflect.Value
		...
		},
		{
			...
			kind: reflect.String,
			wantErr: func(t assert.TestingT, err error, args ...interface{}) bool {
				return assert.EqualError(t, err, "values have wrong kind: expected string, got int")
			},
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err := checkTypesMatchWithKind(tt.v1, tt.v2, tt.kind)
			tt.wantErr(t, err)
		})
	}
}
